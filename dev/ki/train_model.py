...
# Options: 'rf' (Random Forest), 'svm' (Support Vector Machine), 'knn' (K-Nearest Neighbors), 'mlp' (Multi Layer Perceptron), 'gb' (Gradient Boosting)
MODEL_TYPE = 'rf'
USE_SCALER = False    # Set to True to use StandardScaler on the window
    ...
    for f in unique_files:
        ...
        for i in range(n_samples):
            ...
            stats_hf, win_hf = process_signal_fast(
                data_hf[i], win_len, p_mask=HF_P_MASK, v_start=HF_V_START)
            stats_lf, win_lf = process_signal_fast(
                data_lf[i], win_len, p_mask=LF_P_MASK, v_start=LF_V_START)
...
if __name__ == "__main__":
    print(
        f"Configuration: Model={MODEL_TYPE}, Scaler={USE_SCALER}, PCA={USE_PCA} (Var={PCA_VARIANCE})")
    ...
    try:
        ...
        # 2. Extract Features
        X_all, Y_all, raw_hf_all, raw_lf_all, files_all = extract_features(
            df_all, WINDOW_LEN)
    ...
    # 4. Feature Processing Splits
        ...
        if USE_PCA:
            ...
            print(
                f" -> {pca.n_components_} components explain {np.sum(pca.explained_variance_ratio_)*100:.1f}% variance.")
        ...
        if USE_SCALER:
            ...
            config_str += "_noscaling"

        if USE_PCA:
            config_str += f"_pca{int(PCA_VARIANCE*100) if PCA_VARIANCE < 1 else PCA_VARIANCE}"
        else:
            config_str += "_nopca"
        ...
        plot_signal_details(
            ax1, hf_train[0], WINDOW_LEN, HF_P_MASK, HF_V_START, f"HIGH Frequency - {sample_label}")
        plot_signal_details(
            ax2, lf_train[0], WINDOW_LEN, LF_P_MASK, LF_V_START, f"LOW Frequency - {sample_label}")
        plt.tight_layout()
        plt.show()

        # Figure 2: Data Distribution (Train/Test Split)
        train_counts = pd.Series([os.path.basename(f)
                                 for f in files_train]).value_counts()
        test_counts = pd.Series([os.path.basename(f)
                                for f in files_test]).value_counts()
        df_dist = pd.DataFrame(
            {'Train': train_counts, 'Test': test_counts}).fillna(0).sort_index()

        plt.figure(figsize=(10, 6))
        df_dist.plot(kind='bar', stacked=True, color=[
                     '#3498db', '#e74c3c'], alpha=0.8)
